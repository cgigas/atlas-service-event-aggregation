...
import atlas.ssaevent.crud.graphql.Event;
...
public class EventParserTest
{
    ...
    public void testToGraphqlClient()
    {
        ...
        atlas.ssaevent.crud.graphql.Event event = new Event();
        event.setClassificationMarking("");
        event.setPredecessorEventUuid("");
        event.setType(EventType.BREAKUP);
        event.setName("");
        event.setStatus(EventStatus.ACTIVE);
        event.setStartDt(OffsetDateTime.now());
        event.setEndDt(OffsetDateTime.now());
        event.setDescription("");
        event.setInternalNotes("");
        event.setEventPostingId("");
        event.setVersion(1L);
        assertNotNull(task.toGraphqlClient(event, true));
    }
}